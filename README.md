...
一个用于生成中文工科实验报告的 Codex Skill。它面向实验指导书、实验截图、结果曲线、波形图、学生笔记和学校 Word 模板，帮助 Codex 输出更接近真实提交稿的实验报告。

这个 skill 的核心原则很简单：只写材料能支持的内容，不编造实验数据，不把结果和分析混在一起，并把图片放在能证明对应文字的位置。

功能特点
根据实验指导书、截图、图表、波形和笔记整理中文实验报告
支持 Markdown 报告正文和提交版 .docx 两种输出
在 Word 输出中继承学校模板封面和页面格式
自动把正文一级标题转换为 一、实验目的、二、实验过程、三、实验结果、四、实验结果分析
将实验结果截图、原理图、波形图放在相关段落附近，而不是默认堆到附录
按明确条件分组整理结果，例如电阻值、器件尺寸、结构类型等
严格区分 实验结果 和 实验结果分析
...
assets/default-template/实验模板.docx：没有用户模板时使用的默认 Word 模板。
scripts/generate_lab_report_docx.py：将 Markdown 报告正文转换为 .docx 的脚本。
安装
...
使用方式
在 Codex 中直接说明要使用这个 skill，并提供实验材料：
...
License
请在开源前添加许可证文件，例如 LICENSE。如果你还没有确定许可证，建议先不要在 README 中声明具体授权类型。
